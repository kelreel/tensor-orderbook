...
import { shiftPriceByDelta, getLamportsSolBalances } from "./utils";

const COLLECTION_UUID = "ac3c5fb8-e8b5-41e5-b744-7df52b38ddbd"; // retardio cousins
...
export const getPools = async () => {
  ...
  console.log(`${bids.length} bids, ${listings.length} listings`);

  writeFile(`./bids-${new Date().toISOString()}.json`, JSON.stringify(bids, null, 2), (err) => {
    if (err) {
      console.error(err);
    }
  });
